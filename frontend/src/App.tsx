...
function Login({onLogin}:{onLogin:(u:User)=>void}){
  const [email,setEmail]=useState('admin@devflow.com'),[password,setPassword]=useState('admin123'),[error,setError]=useState(''),[loading,setLoading]=useState(false);
...
function Dashboard({data,projects,user,onNew}:{data:DashboardData|null;projects:Project[];user:User;onNew:()=>void}){
...
function Kanban({tasks,projects,onMove,onNew}:{tasks:Task[];projects:Project[];onMove:(id:number,s:TaskStatus)=>void;onNew:()=>void}){
...
export default function App(){
 ...
 const content=page==='Dashboard'?<Dashboard data={dashboard} projects={projects} user={user} onNew={()=>setModal('project')}/>:page==='Kanban'||page==='Tarefas'?<Kanban tasks={tasks} projects={projects} onMove={move} onNew={()=>setModal('task')}/>:page==='Projetos'?<Projects projects={projects} tasks={tasks} onNew={()=>setModal('project')}/>:<Placeholder page={page}/>;
 return <div className="app"><Sidebar page={page} setPage={setPage} open={open} setOpen={setOpen} user={user} onLogout={logout}/><main><Header page={page} onMenu={()=>setOpen(true)}/>{error&&<div className="error app-error" role="alert"><span>{error}</span><button className="secondary" disabled={loading} onClick={()=>void load()}>{loading?'Carregando...':'Atualizar dados'}</button></div>}{!dashboard?(loading?<Loading/>:null):content}</main>{modal&&<Modal kind={modal} projects={projects} onClose={()=>setModal(null)} onSessionExpired={logout} onDone={()=>{setModal(null);void load()}}/>}</div>
}
